...
pub async fn get_trackable_by_imei(configuration: &configuration::Configuration, params: GetTrackableByImeiParams) -> Result<models::Trackable, Error<GetTrackableByImeiError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Trackable`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Trackable`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetTrackableByImeiError> = serde_json::from_str(&content).ok();
