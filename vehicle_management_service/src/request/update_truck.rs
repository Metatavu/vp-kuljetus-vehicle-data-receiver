...
/**You should use this struct via [`VehicleManagementServiceClient::update_truck`].
...
pub struct UpdateTruckRequired<'a> {
    pub plate_number: &'a str,
    pub truck_id: &'a str,
    pub type_: &'a str,
    pub vin: &'a str,
}
impl<'a> UpdateTruckRequired<'a> {}
impl FluentRequest<'_, UpdateTruckRequest> {
    ///Set the value of the active_vehicle_id field.
    pub fn active_vehicle_id(mut self, active_vehicle_id: &str) -> Self {
        self.params.active_vehicle_id = Some(active_vehicle_id.to_owned());
        self
    }
...
impl<'a> ::std::future::IntoFuture for FluentRequest<'a, UpdateTruckRequest> {
    ...
    fn into_future(self) -> Self::IntoFuture {
        Box::pin(async move {
            ...
            if let Some(ref unwrapped) = self.params.active_vehicle_id {
                r = r.json(json!({ "activeVehicleId" : unwrapped }));
            }
