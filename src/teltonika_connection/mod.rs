...
use crate::{
    teltonika_handler::teltonika_records_handler::TeltonikaRecordsHandler,
            ...
            match self.teltonika_stream.read_frame_async().await {
                Ok(frame) => {
                    ...
                    if let None = truck_vin {
                        truck_vin =
                            teltonika_records_handler.get_truck_vin_from_records(&frame.records);
                    }
                ...
                Err(err) => match err.kind() {
                    std::io::ErrorKind::ConnectionReset => {
                        info!(
                            "Client with IMEI [{}] disconnected",
                            self.imei.as_ref().unwrap()
                        );
                        break;
                    }
                    std::io::ErrorKind::InvalidData => {
