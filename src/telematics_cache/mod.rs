...
pub trait Cacheable {
  ...
  /// # Arguments
  /// * `base_cache_path` - The base path to the cache directory
  fn write_to_file(&self, base_cache_path: &str) -> Result<(), std::io::Error> where Self: Serialize + Sized + for<'a>Deserialize<'a> + Clone {
  ...
  fn read_from_file(base_cache_path: &str) -> Vec<Self> where Self: Sized + for<'a>Deserialize<'a> {
